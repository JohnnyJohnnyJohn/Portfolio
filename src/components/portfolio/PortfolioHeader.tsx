...
export function PortfolioHeader({
  ...
  return (
    <header className="fixed  pointer-events-none inset-x-0 top-0 z-30 px-4 pt-5 md:px-6 md:pt-8 min-[1440px]:px-12">
      <div className="relative mx-auto flex items-center justify-between gap-4">
        <div className="min-w-0">
          ...
          <div className="pointer-events-auto md:pointer-events-none md:hidden">
            <SegmentedSwitch
              value={activeView}
              options={viewOptions}
        ...
        <div className="absolute top-0 left-1/2 pointer-events-none hidden -translate-x-1/2 min-[1440px]:block min-[1440px]:pointer-events-auto">
          <SegmentedSwitch
            value={activeView}
            options={viewOptions}
            onChange={onViewChange}
        ...
        {isMenuOpen ? (
          <div className="absolute top-16 right-0 w-40 p-1.5 rounded-3xl pointer-events-auto border border-portfolio-border bg-linear-to-b from-white/8 to-white/2 text-white shadow-[0_18px_70px_rgba(0,0,0,.45)] backdrop-blur-xl transition-colors hover:border-white/20 hover:from-white/12 min-[1440px]:hidden">
...
function HeaderLink({ children, href }: { children: ReactNode; href: string }) {
  return (
    <a
      href={href}
      className="inline-flex items-center gap-2 rounded-full py-3.5 px-4 backdrop-blur-lg transition-colors duration-300 hover:bg-linear-to-br hover:from-white/15 hover:to-transparent"
    >
      {children}
...
function MobileMenuLink({
  ...
  return (
    <a
      href={href}
      className="flex items-center justify-between rounded-full w-full h-10 py-3 px-4.5 text-sm font-medium text-white transition-colors hover:bg-linear-to-br hover:from-white/15 hover:to-transparent"
    >
      {children}
